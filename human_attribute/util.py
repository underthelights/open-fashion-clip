...
from module.human_attribute.bbox import bbox_iou, bbox_iou2
import sys
...
def write(x, img, phrases, order, coco_classes, colors):
    c1 = x[1:3].int().cpu().numpy()
    c2 = x[3:5].int().cpu().numpy()
    # c1 = int(np.float32(c1))
    # c2 = int(np.float32(c2))
    ...
    if cls == 0:  # for human bbox only detection
        ...
        color = random.choice(colors)
        cv2.rectangle(img, c1, (int(np.float32(c2[0])), int(np.float32(c2[1]))), color, 3)
        t_size = cv2.getTextSize(label, cv2.FONT_HERSHEY_PLAIN, 1 , 1)[0]
        c2 = c1[0] + t_size[0] + 3, c1[1] + t_size[1] + 4
        # cv2.rectangle(img, c1, c2, color, -1)
