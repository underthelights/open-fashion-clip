...
def main(args):
    ...
    try:
     ...
     #   list_dir.sort(key=lambda x: int(x[:-4]))
        imlist = [osp.join(osp.realpath('Air-Clothing-MA'), images, img) for img in list_dir if os.path.splitext(img)[1] == '.jpg' or os.path.splitext(img)[1] == '.JPG' or os.path.splitext(img)[1] == '.png']
    except NotADirectoryError:
        imlist = []
        imlist.append(osp.join(osp.realpath('Air-Clothing-MA'), images))
